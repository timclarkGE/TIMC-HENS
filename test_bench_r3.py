...
pipe_r = 10  # Fake value
track_h = 1  # Fake value
tool_height_j2 = 2.959  # Fake value
a1 = pipe_r + track_h + tool_height_j2  # Value as reported by Jason Mann
a2 = 12.0
a3 = 15.0
a4 = 1.1  # RPV Parallel gimbal component
a5 = 5.0  # RPV Perpendicular component
...
min_reach = 16.93
scan_res = 0.05  # Inches
scan_vel = 12.0  # Inches/second
scan_length = 13.8  # Inches 13.8 default
scan_index_angle = 10  # Degrees, 1.8, TODO: this breaks down are large angles
...
def c_angles(pt, cfg):
    ...
    # Calculate major and minor axes of ellipse generated by slicing the RPV according to t1
    ...
    # New rotated scan point, TODO: consolidate this statement
...
def calc_scan_pts(scan_length, index_angle):
    ...
    for i in range(0, total_pts):
        ...
        for j in range(0, int(scan_length / scan_res)):
            ...
            pa = append(pa, array([[new_x], [new_y], [new_z], [1]]), axis=1)
        ...
        # Create vector from start of arc to end, TODO: make actual arc and not straight line
        ...
        for j in range(0, int(vec_mag / scan_res)):
            ...
            # f.write(str(round(new_x, 3)) + " " + str(round(new_y, 3)) + " " + str(round(new_z, 3)) + "\n")
            pa = append(pa, array([[new_x], [new_y], [new_z], [0]]), axis=1)
...
# NOT USED: Function to read in scan points from file. This function takes longer than creating the points at runtime
def read_scan_point_file():
    filename = ["scan_points_1.8_0.1.txt", "scan_points_1.8_0.05.txt", "scan_points_1.8_0.025.txt"]
    pa = array([[], [], []])

    with open(filename[0], 'r') as f:
        for line in f:
            pa = append(pa, array([[line.split()[0]], [line.split()[1]], [line.split()[2]]]))
    f.close()
...
rpv = vp.cylinder(pos=vp.vector(-rpv_h / 2, 0, rpv_r + rpv_offset), axis=vp.vector(rpv_h, 0, 0), radius=rpv_r,
                  color=vp.color.cyan)
scan_pipe = vp.cylinder(pos=vp.vector(0, 0, 0.1), axis=vp.vector(0, 0, rpv_offset), radius=14.25 / 2,
                        color=vp.color.red)
scan_boss = vp.cylinder(pos=vp.vector(0, 0, rpv_offset), axis=vp.vector(0, 0, -5.75), radius=25.25 / 2,
                        color=vp.color.red)
...
# Initialize communication with position blaster
# TODO: https://pyserial.readthedocs.io/en/latest/tools.html look into serial adapter ID
...
for j in range(num_pts):
    ...
    if sim_true_scan_speed:
        while (time_2 - time_1) < scan_res / scan_vel:
            time_2 = time.time()
        time_1 = time_2
    ...
    if scan_pts[3, j] == 1:
        ...
        ser.write(chr(96).encode())
        print(ser.read())

    # # Code to calculate max commanded speed
